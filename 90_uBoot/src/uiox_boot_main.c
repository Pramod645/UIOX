/*
 * uiox_boot_main.c - 7-stage UIOX bootloader pipeline.
 */
#include "uiox_boot.h"
#include <string.h>
...
#define KBUF_SIZE (16u*1024u*1024u)
static uboot_u8_t g_kbuf[KBUF_SIZE] __attribute__((aligned(4096)));
...
int uiox_boot_main(uboot_u64_t dtb, uboot_u32_t flags)
{
    ...
    /* init heap in first usable region */
    uboot_heap_init(g_mmap.regions[0].base + 0x100000u,
                    UIOX_BOOT_HEAP_SIZE);

    /* Stage 3: Storage */
    uboot_puts("[BOOT] Stage 3: Storage\r\n");
    rc = uboot_blk_probe_emmc(&g_blk);
    if(rc!=UBOOT_OK) rc = uboot_blk_probe_nvme(&g_blk,0);
    if(rc!=UBOOT_OK) rc = uboot_blk_probe_usb(&g_blk);
    if(rc==UBOOT_OK) {
        rc = uboot_fat32_init(&g_fat32, &g_blk, 2048u);
        if(rc==UBOOT_OK) uboot_puts("  FAT32 mounted\r\n");
    } else {
    ...
    rc = uboot_fat32_load(&g_fat32, UIOX_KERNEL_FILENAME,
                           g_kbuf, KBUF_SIZE, &ksize);
    if(rc==UBOOT_ENODEV || rc==UBOOT_ENOENT) {
        uboot_puts("  No kernel file — QEMU sim handoff\r\n");
        goto handoff;
    }
    ...
    if(rc==UBOOT_EVERIFY) panic("SHA-256 mismatch!");
    uboot_puts("  Verify OK\r\n");

handoff:;
    /* Stage 6: ELF load */
    uboot_puts("[BOOT] Stage 6: ELF load\r\n");
    uboot_u64_t entry = 0;
...
#endif
        if(ksize) uboot_memcpy((void*)(uboot_addr_t)entry,
                                g_kbuf, ksize);
    }
    uboot_printf("  Entry: "); uboot_puthex64(entry);
    uboot_puts("\r\n");
    ...
    g_args.arch         = arch_id();
    g_args.mem_map      = g_mmap;
    g_args.kernel_entry = entry;
    g_args.kernel_size  = ksize;
    g_args.dtb_phys     = dtb;
    g_args.uart_baud    = UIOX_UART_DEFAULT_BAUD;
#if   defined(__aarch64__)
    g_args.uart_type = UBOOT_UART_PL011;
    g_args.uart_base = 0x09000000ULL;
#elif defined(__arm__)
    g_args.uart_type = UBOOT_UART_PL011;
    g_args.uart_base = 0x10009000ULL;
#else
    g_args.uart_type = UBOOT_UART_16550;
    g_args.uart_base = 0x3F8u;
#endif
    const char *cl = UIOX_CMDLINE_DEFAULT;
    uboot_size_t ci = 0;
    while(cl[ci] && ci < UIOX_BOOT_CMDLINE-1u)
        { g_args.cmdline[ci]=cl[ci]; ci++; }
    g_args.cmdline[ci]='\0';
    uboot_args_checksum(&g_args);
    uboot_args_print(&g_args);

    uboot_u64_t args_phys;
#if   defined(__aarch64__)
    args_phys = UIOX_BOOT_ARGS_PHYS_ARM64;
#elif defined(__arm__)
    args_phys = UIOX_BOOT_ARGS_PHYS_ARM32;
#else
    args_phys = UIOX_BOOT_ARGS_PHYS_X86;
#endif
    uboot_memcpy((void*)(uboot_addr_t)args_phys,
                 &g_args, sizeof(g_args));
    uboot_mem_barrier();

    uboot_puts("[BOOT] Jumping to kernel...\r\n\r\n");
    uboot_jump_to_kernel(entry, args_phys);
    return 0;
}
