 ...
 */
                              ...
                              const uiox_tpwd_raw_evt_t *ev)
 {
     ...
     rb->buf[rb->head] = *ev; rb->head = next; return true;
 }
 
 bool uiox_tpwd_evtbuf_pop(uiox_tpwd_evtbuf_t *rb, uiox_tpwd_raw_evt_t *ev)
 {
 ...
 void uiox_tpwd_cred_pool_init(void)
 {
     ...
     for (int i = 0; i < UIOX_TPWD_CRED_POOL_SIZE; i++) {
         ...
         s_cred_free = &s_cred_pool[i]; s_cred_cnt++;
     }
 }
 ...
 uiox_tpwd_cred_t *uiox_tpwd_cred_alloc(void)
 {
     ...
     s_cred_free = c->next; s_cred_cnt--;
     c->next = NULL; c->in_use = 1; c->attempts = 0;
     return c;
 }
 ...
 void uiox_tpwd_cred_free(uiox_tpwd_cred_t *c)
 {
     ...
     c->attempts = 0; c->in_use = 0;
     c->next = s_cred_free; s_cred_free = c; s_cred_cnt++;
 }
