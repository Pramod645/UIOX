...
A CRC32C table and the bit-by-bit loop compute the identical result. The table just precomputes 
the 8 rounds of the inner loop for every possible byte, so you do one lookup instead of eight shifts. 
Same polynomial (0x82F63B78), same 0xFFFFFFFF init/final XOR — a table-driven CRC and a bitwise CRC are 
two ways to evaluate the same function.

That means once unfs_crc32c() uses the loop, the table is genuinely dead code — nothing reads it. 
and only the loop is referenced. Removing it clears the warning and changes no output byte.
                       ...
                       unfs_inode_t       *out)
{
    ...
    uint32_t local_idx, blk_local;
    uint32_t grp        = udiv32_softp(ino0, mnt->sb.s_inodes_per_group, &local_idx);
    uint32_t blk_offset = udiv32_softp(local_idx, UNFS_INODES_PER_BLOCK, &blk_local);
    #else
    uint32_t ino0       = ino - 1u;
 ...
 * ───────────────────────────────────────────────────────────────────── */
 static int dir_lookup(const unfs_inode_t *dir_inode,
                       const char         *name,
                       uint8_t             name_len)
{
    ...
    for (uint32_t b = 0u; b < n_blks; b++) {
        uint32_t phys = extent_lookup(dir_inode, b);
                ...
                unfs_inode_t *inode_out)
{
    ...
    while (*p != '\0') {
        ...
        int child_ino = dir_lookup(&cur_inode,
                                   comp_start, comp_len);
                    ...
                    uint64_t           *bytes_read_out)
{
    ...
    while (remaining > 0u) {
        ...
        uint32_t phys = extent_lookup(inode, logical_blk);
