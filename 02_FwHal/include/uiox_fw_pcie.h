/**
 * @file  uiox_fw_pcie.h
 * @brief UIOX Firmware HAL — PCIe early initialisation.
 *
 * Handles PCIe config space access, BAR assignment, and link training
 * for NVMe SSDs, SATA controllers, and network cards during firmware
 * init before the kernel takes over PCI enumeration.
 *
 * @version 1.0.0
 * @date    2026-07-07
 */

 #ifndef UIOX_FW_PCIE_H
 #define UIOX_FW_PCIE_H
 ...
 /* =========================================================================
  * PCIe config space access (MMIO ECAM — PCI Express Base Spec §7.2)
  * ====================================================================== */
 
 /* ECAM base addresses */
 #define UIOX_PCIE_ECAM_ARM64    0x3F000000u  /**< QEMU virt PCIe ECAM   */
 #define UIOX_PCIE_ECAM_X86_Q35  0xB0000000u  /**< QEMU q35 PCIe ECAM    */
 
 /* ECAM offset formula:
  *   base + ((bus << 20) | (dev << 15) | (fn << 12) | offset) */
 #define UIOX_PCIE_ECAM_ADDR(ecam, bus, dev, fn, off) \
     ((ecam) + (((uint32_t)(bus) << 20u) | ((uint32_t)(dev) << 15u) | \
                ((uint32_t)(fn)  << 12u) | (uint32_t)(off)))
 ...
 #define UIOX_PCIE_MAX_DEVICES   32u
 ...
 typedef struct {
     ...
     uiox_pcie_dev_t  devices[UIOX_PCIE_MAX_DEVICES];
 ...
 uiox_fw_err_t uiox_fw_pcie_init      (uiox_pcie_ctrl_t *ctrl,
                                         uintptr_t ecam_base);
