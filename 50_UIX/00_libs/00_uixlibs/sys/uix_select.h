...
#define UIX_FD_SETSIZE 1024

typedef struct {
    uix_uint64_t fds_bits[UIX_FD_SETSIZE / 64];
} uix_fd_set;

#define UIX_FD_ZERO(s)     uix_memset((s), 0, sizeof(uix_fd_set))
#define UIX_FD_SET(fd,s)   ((s)->fds_bits[(fd)/64] |=  (1ULL<<((fd)%64)))
#define UIX_FD_CLR(fd,s)   ((s)->fds_bits[(fd)/64] &= ~(1ULL<<((fd)%64)))
#define UIX_FD_ISSET(fd,s) (!!((s)->fds_bits[(fd)/64] & (1ULL<<((fd)%64))))
