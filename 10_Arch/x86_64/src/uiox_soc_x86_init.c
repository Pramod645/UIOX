 ...
 */
#include "../include/uiox_soc_x86.h"
...
static void x86_ioapic_init(void)
{
    ...
    uint32_t max_redir = (ver >> 16u) & 0xFFu;
    ...
    for (uint32_t i = 0u; i <= max_redir; i++)
        ioapic_write((uint8_t)IOAPIC_REG_REDTBL(i),
                     IOAPIC_RED_MASKED | (SOC_IOAPIC_IRQ_BASE + i));

    /* Unmask COM1 (IRQ4) → vector SOC_IOAPIC_IRQ_BASE + 4 */
    uint32_t uart_vec = (uint32_t)SOC_IOAPIC_IRQ_BASE + SOC_UART_IRQ;
    ioapic_write((uint8_t)IOAPIC_REG_REDTBL(SOC_UART_IRQ), uart_vec);
...
static void x86_lapic_timer_init_hpet(uint32_t hz)
{
    /* 1. Program HPET timer 0 in one-shot mode for 10 ms */
    uint64_t hpet_period_fs =
        ((uint64_t)mmio_read32(HPET_GCAP_ID + 4u)) & 0xFFFFFFFFull;
    ...
    uint64_t hpet_10ms = 10000000000000ull / hpet_period_fs;

    /* Disable LAPIC timer, set divisor to 16 */
    mmio_write32(LAPIC_LVT_TIMER, LAPIC_LVT_MASKED);
    mmio_write32(LAPIC_TIMER_DCR, 0x3u);   /* divide by 16               */
    mmio_write32(LAPIC_TIMER_ICR, 0xFFFFFFFFu);

    /* Start HPET */
    uint64_t start_ctr = mmio_read64(HPET_MAIN_CTR);
    mmio_write32(HPET_GEN_CFG, HPET_CFG_ENABLE);

    /* Wait 10 ms */
    while ((mmio_read64(HPET_MAIN_CTR) - start_ctr) < hpet_10ms)
        __asm__ volatile("pause");

    /* Read how many LAPIC ticks elapsed in 10 ms */
    uint32_t ticks_10ms = 0xFFFFFFFFu - mmio_read32(LAPIC_TIMER_CCR);
    uint32_t ticks_per_sec = ticks_10ms * 100u;
    uint32_t lapic_period  = ticks_per_sec / hz;

    printf("[soc/x86]  LAPIC timer: %u ticks/s => period=%u ticks @ %u Hz\n",
           ticks_per_sec, lapic_period, hz);

    /* Program LAPIC periodic timer */
    mmio_write32(LAPIC_TIMER_DCR, 0x3u);
    mmio_write32(LAPIC_LVT_TIMER, LAPIC_TIMER_PERIODIC
                                 | (SOC_IOAPIC_IRQ_BASE + 0u));
    mmio_write32(LAPIC_TIMER_ICR, lapic_period);
}

/* ── 8253/8254 PIT — legacy init (10 ms period, channel 0) ─────────── */
static void x86_pit_init(uint32_t hz)
{
    uint32_t divisor = 1193182u / hz;  /* PIT input = 1.193182 MHz        */
    _outb(SOC_PIT_PORT + 3u, 0x34u);  /* Channel 0, rate generator, lo/hi */
    _outb(SOC_PIT_PORT,      (uint8_t)(divisor & 0xFFu));
    _outb(SOC_PIT_PORT,      (uint8_t)((divisor >> 8u) & 0xFFu));
