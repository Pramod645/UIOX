/*
 * 30_KIX/33_PCS/01_schedular/src/syscall_time.c
 *
 ...
 *   REMOVED: #include <stdlib.h>  → NULL via uiox_klibc.h
 *   Both flow in through syscall_time.h → sched_types.h → uiox_klibc.h
 *   so no explicit include is needed here.
 *
 ...
 */
 ...
 /* stdio.h / string.h / stdlib.h removed — all provided transitively  */
 /* through syscall_time.h → sched_types.h → uiox_klibc.h              */
 ...
 int sys_setitimer(Process *p, const ItimerVal *new_val, ItimerVal *old_val)
 {
     ...
     if (old_val) {
         unsigned int rem = sys_alarm(p, 0);
 ...
 static void posix_timer_expire(unsigned long data)
 {
     ...
     printf("[posix_timer] expired id=%d  clock=%s\n",
            idx,
            pt->clock_id == UIOX_CLOCK_REALTIME ? "REALTIME" : "MONOTONIC");
