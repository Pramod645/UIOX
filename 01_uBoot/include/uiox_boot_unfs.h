 ...
 */
 ...
 struct unfs_inode {
      ...
      * Everything above sums to exactly 168 bytes:
      *
      ...
      * (device numbers for IFCHR/IFBLK — 01_fsa's mknod records that it
      * has no slot for them), i_generation and i_dtime are the fields
      * that plausibly belong here, but no consumer is written against
      * them, so inventing offsets now would be guessing at layout.
      *
      * CONSEQUENCE, stated plainly: an inode image written today carries
      * 88 bytes that mean nothing.  That is the cost of keeping the
