...
static void gic600_redist_wakeup(uiox_uint32_t cpu)
{
    uiox_uint64_t waker_reg = GIC600_REDIST_WAKER(cpu);
    uiox_uint32_t waker     = soc_mmio_read32(waker_reg);

    /* Clear ProcessorSleep (bit 1) */
    waker &= ~(1u << 1u);
    soc_mmio_write32(waker_reg, waker);
    //arch_dsb();

    /* Poll ChildrenAsleep (bit 2) until cleared — max 1000 iterations */
    for (int i = 0; i < 1000; i++) {
        if (!(soc_mmio_read32(waker_reg) & (1u << 2u))) break;
        //arch_isb();
    }
...
static void arm64_cache_topology(void)
{
    ...
    for (uiox_uint64_t level = 0u; level < loc && level < 7u; level++) {
        ...
        const char *type_str[] = { "none", "i-only", "d-only",
                                   ...
                                   "res5", "res6", "res7" };
        /* Select cache level */
        uiox_uint64_t csselr = ((uiox_uint64_t)level << 1u);
        __asm__ volatile("msr csselr_el1, %0" :: "r"(csselr) : "memory");
        //arch_isb();

        uiox_uint64_t ccsidr;
        __asm__ volatile("mrs %0, ccsidr_el1" : "=r"(ccsidr));
        uiox_uint32_t sets  = (uiox_uint32_t)(((ccsidr >> 13u) & 0x7FFFu) + 1u);
        uiox_uint32_t ways  = (uiox_uint32_t)(((ccsidr >>  3u) & 0x3FFu)  + 1u);
...
{
    uiox_uint64_t mpidr, midr;
    __asm__ volatile("mrs %0, mpidr_el1" : "=r"(mpidr));
    __asm__ volatile("mrs %0, midr_el1"  : "=r"(midr));

    uiox_uint32_t cluster, core;
    arm64_mpidr_decode(mpidr, &cluster, &core);

    uiox_uint32_t impl    = (uiox_uint32_t)((midr >> 24u) & 0xFFu);
    uiox_uint32_t partnum = (uiox_uint32_t)((midr >>  4u) & 0xFFFu);
    uiox_uint32_t rev     = (uiox_uint32_t)((midr >>  0u) & 0xFu);
    uiox_uint32_t var     = (uiox_uint32_t)((midr >> 20u) & 0xFu);
...
int uiox_soc_arm64_init(void)
{
    ...
    for (uiox_uint32_t i = 0u; i < desc->num_cpus && i < 8u; i++)
