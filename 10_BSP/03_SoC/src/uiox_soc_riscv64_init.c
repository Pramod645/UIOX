...
static void rv_delegation_init(void)
{
     ...
     */
    uiox_uint64_t medeleg = (1u <<  0) | (1u <<  1) | (1u <<  2) | (1u <<  3)
                     | (1u <<  4) | (1u <<  5) | (1u <<  6) | (1u <<  7)
     ...
     */
    uiox_uint64_t mideleg = (1u << 1) | (1u << 5) | (1u << 9);
...
{
    uiox_uint32_t cfg = soc_mmio_read32(SIFIVE_L2CC_CONFIG);
    if (cfg == 0u || cfg == 0xFFFFFFFFu) {
        printf("[soc/riscv] SiFive L2 cache controller not present\n");
        return;
    }

    uiox_uint32_t banks     = (cfg >>  0u) & 0xFFu;
    uiox_uint32_t ways      = (cfg >>  8u) & 0xFFu;
    uiox_uint32_t sets_log2 = (cfg >> 16u) & 0xFFu;
    uiox_uint32_t size_kb   = banks * ways * (1u << sets_log2) * 64u / 1024u;

    /* Enable all ways */
    soc_mmio_write32(SIFIVE_L2CC_WAYS, (1u << ways) - 1u);
    //arch_mb();
...
static void rv_clint_multi_hart_init(uiox_uint32_t num_harts)
{
    for (uiox_uint32_t h = 0u; h < num_harts && h < 8u; h++) {
        /* Clear software interrupt */
        //soc_mmio_write32(CLINT_MSIP(h), 0u);
        /* Set MTIMECMP far in future (no spurious timer on secondary harts) */
        //soc_mmio_write32(CLINT_MTIMECMP(h),       0xFFFFFFFFu);
        //soc_mmio_write32(CLINT_MTIMECMP(h) + 4u,  0xFFFFFFFFu);
    }
...
{
    uiox_uint64_t misa     = 0;//CSR_READ("misa");
    uiox_uint64_t mstatus  = 0;//CSR_READ("mstatus");
    uiox_uint64_t mhartid  = 0;//CSR_READ("mhartid");
...
int uiox_soc_riscv64_init(void)
{
    ...
    uiox_uint32_t num_harts = desc ? desc->num_cpus : 1u;
