/**
 * @file    uiox_cam_buf.c
 * @brief   UIOX Camera frame buffer pool implementation.
 */
 ...
 static uintptr_t to_phys(void *v)
 {
     uintptr_t r;
     memcpy(&r, &v, sizeof(r));
     return r;
                         ...
                         uint32_t stride, uint8_t fmt)
 {
     ...
     for (int i = 0; i < UIOX_CAM_POOL_FRAMES; i++) {
         ...
         memcpy(&base, &mem_i, sizeof(base));
         uintptr_t aligned = (base + (UIOX_CAM_FRAME_ALIGN - 1))
                             & ~(uintptr_t)(UIOX_CAM_FRAME_ALIGN - 1);
         uint8_t *vaddr_ptr;
         memcpy(&vaddr_ptr, &aligned, sizeof(vaddr_ptr));
         f->vaddr = vaddr_ptr;
         f->paddr = to_phys(vaddr_ptr);
         f->length = UIOX_CAM_FRAME_MAX;
         f->width = s_w; f->height = s_h;
         f->stride = s_stride; f->fmt = s_fmt;
         f->next = s_free; s_free = f; s_free_count++;
     }
 }
 ...
 uiox_cam_frame_t *uiox_cam_buf_alloc(void)
 {
     ...
     s_free = f->next; s_free_count--;
     f->next = NULL; f->in_use = 1;
     return f;
 }
 
 void uiox_cam_buf_ref(uiox_cam_frame_t *f) { if (f) f->in_use++; }
 
 void uiox_cam_buf_free(uiox_cam_frame_t *f)
 {
     if (!f) return;
     UIOX_ASSERT(f->in_use > 0);
     if (--f->in_use == 0) { f->next = s_free; s_free = f; s_free_count++; }
 }
