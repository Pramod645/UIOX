/**
 * @file  uiox_fw_dma.c
 * @brief UIOX Firmware HAL — DMA controller (software-fallback driver).
 *        Provides a working DMA API backed by CPU copy when no real
 *        DMA hardware is present (QEMU simulation mode).
 * @date  2026-07-07
 */

 #include "../include/uiox_fw_dma.h"

 #define OPS(c) ((const uiox_dma_ops_t *)(c)->priv)
 
 /* =========================================================================
  * Software fallback DMA (CPU copy — no real DMA hardware needed)
  * ====================================================================== */
 
 static uiox_fw_err_t sw_dma_init(uiox_dma_ctrl_t *ctrl)
 {
     ctrl->num_channels = UIOX_DMA_MAX_CHANNELS;
     ctrl->initialized  = true;
     return UIOX_FW_OK;
 }
 
 static void sw_dma_deinit(uiox_dma_ctrl_t *ctrl)
 { ctrl->initialized = false; }
 
 static uiox_fw_err_t sw_dma_transfer(uiox_dma_ctrl_t *ctrl,
                                        uint32_t chan,
                                        const uiox_dma_xfer_t *xfer)
 {
     if (chan >= UIOX_DMA_MAX_CHANNELS) return UIOX_FW_ERR_INVAL;
     if (ctrl->chan_busy[chan])         return UIOX_FW_ERR_BUSY;
     ctrl->chan_busy[chan] = true;
 
     /* Software copy respecting width and increment flags */
     if (xfer->width == UIOX_DMA_WIDTH_32) {
     ...
     } else if (xfer->width == UIOX_DMA_WIDTH_16) {
 ...
 static bool  sw_dma_busy (uiox_dma_ctrl_t *ctrl, uint32_t chan)
 { return chan < UIOX_DMA_MAX_CHANNELS ? ctrl->chan_busy[chan] : false; }
 
 static uiox_fw_err_t sw_dma_wait(uiox_dma_ctrl_t *ctrl, uint32_t chan)
 {
     /* Software DMA completes synchronously — already done */
     (void)ctrl; (void)chan; return UIOX_FW_OK;
 }
 
 static void sw_dma_abort(uiox_dma_ctrl_t *ctrl, uint32_t chan)
 { if (chan < UIOX_DMA_MAX_CHANNELS) ctrl->chan_busy[chan] = false; }
 
 static int sw_dma_alloc_chan(uiox_dma_ctrl_t *ctrl)
 {
     for (uint32_t i = 0u; i < UIOX_DMA_MAX_CHANNELS; i++) {
         if (!ctrl->chan_busy[i]) return (int)i;
     }
     return -1;
 }
 
 static void sw_dma_free_chan(uiox_dma_ctrl_t *ctrl, uint32_t chan)
 { if (chan < UIOX_DMA_MAX_CHANNELS) ctrl->chan_busy[chan] = false; }
 
 static void sw_dma_isr(uiox_dma_ctrl_t *ctrl) { (void)ctrl; }
 
 static const uiox_dma_ops_t sw_dma_ops = {
     .init       = sw_dma_init,
                                   ...
                                   const uiox_dma_ops_t *ops)
 {
     ...
     const uiox_dma_ops_t *use = ops ? ops : &sw_dma_ops;
     ctrl->priv = (void *)use;
     return use->init(ctrl);
 }
 
 void uiox_fw_dma_deinit(uiox_dma_ctrl_t *ctrl)
 { if (ctrl && ctrl->priv && OPS(ctrl)->deinit) OPS(ctrl)->deinit(ctrl); }
 
 uiox_fw_err_t uiox_fw_dma_transfer(uiox_dma_ctrl_t *ctrl, uint32_t chan,
                                       const uiox_dma_xfer_t *xfer)
 {
     if (!ctrl || !ctrl->priv || !xfer) return UIOX_FW_ERR_INVAL;
 ...
 { if (!ctrl || !ctrl->priv) return UIOX_FW_ERR_INVAL;
 ...
 uiox_fw_err_t uiox_fw_dma_memcpy(uiox_dma_ctrl_t *ctrl,
                                     ...
                                     uint32_t len)
 {
     int chan = uiox_fw_dma_alloc_chan(ctrl);
     if (chan < 0) return UIOX_FW_ERR_BUSY;
     uiox_dma_xfer_t x = {
         .src      = src, .dst = dst, .len = len,
         .dir      = UIOX_DMA_DIR_MEM_TO_MEM,
         .width    = UIOX_DMA_WIDTH_32,
         .src_inc  = true, .dst_inc = true,
         .complete_cb = NULL,
     };
     uiox_fw_err_t rc = uiox_fw_dma_transfer(ctrl, (uint32_t)chan, &x);
     if (rc == UIOX_FW_OK) uiox_fw_dma_wait(ctrl, (uint32_t)chan);
     uiox_fw_dma_free_chan(ctrl, (uint32_t)chan);
     return rc;
 }
