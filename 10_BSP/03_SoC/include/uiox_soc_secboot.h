 ...
 */
 ...
 typedef struct {
     uint8_t           pubkey_hash[UIOX_SOC_SECBOOT_SHA256_LEN];
 ...
 typedef struct __attribute__((packed)) {
     ...
     uint32_t version;
     uint32_t flags;
     uint8_t  subject_pubkey[UIOX_SOC_SECBOOT_KEY_MAX_LEN];
     uint32_t subject_pubkey_len;
     uint8_t  issuer_sig[UIOX_SOC_SECBOOT_SIG_MAX_LEN];
 ...
 typedef struct __attribute__((packed)) {
     ...
     uint8_t  image_hash[UIOX_SOC_SECBOOT_SHA256_LEN];
     uint8_t  signature[UIOX_SOC_SECBOOT_SIG_MAX_LEN];
 ...
 typedef struct {
     ...
     uint8_t                   measured_hash[UIOX_SOC_SECBOOT_SHA256_LEN];
 ...
 void           uiox_soc_secboot_extend_pcr   (uint32_t pcr_index,
                                                const uint8_t measurement[32]);
 void           uiox_soc_secboot_read_pcr     (uint32_t pcr_index,
                                                uint8_t out[32]);
 ...
 void uiox_soc_sha256_final (uiox_soc_sha256_ctx_t *ctx,
                               uint8_t digest[32]);
 void uiox_soc_sha256       (const uint8_t *d, size_t l,
                               uint8_t digest[32]);
