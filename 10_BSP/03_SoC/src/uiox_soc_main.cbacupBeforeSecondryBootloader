...
#  endif
     ...
     /* ================================================================ */
     /* Stage 8: Power init + hand-off to kernel                          */
     /* ================================================================ */
     uiox_soc_printf("Stage 8: Handoff → kernel\n");
 
     rc = uiox_soc_power_init(&s_power);
     if (rc != UIOX_SOC_OK) SOC_FATAL_MSG("power_init failed");
 
     /* Sync barriers before kernel entry */
     uiox_soc_hw_dsb();
     uiox_soc_hw_isb();
 
     uiox_soc_printf(UIOX_SOC_VERSION_STR
                      " init complete — jumping to kernel\n");
 
     /* Hand off — kernel must be linked and provide this symbol */
