/*
 *  30_KIX/20_DriverInterfaces/include/uiox_klibc.h
 *
 *  Freestanding C runtime replacement for the UIOX kernel.
 *  Standalone copy — no dependency on 33_PCS or any sibling subsystem.
 *
 *  Replaces: <stdint.h>  <stdbool.h>  <stddef.h>  <string.h>
 ...
 *  §6  String  (strlen, strcmp, strncmp, strcpy, strncpy, strchr)
 *  §7  I/O     (printf — implemented by BSP SoC stdio)
 *  §8  Math    (min, max, abs — integer only, no FPU)
 *  §9  Aliases (memset/memcpy/printf map to uiox_* above)
 */
#ifndef UIOX_KLIBC_H
#define UIOX_KLIBC_H
 ...
 * Replace any fprintf(stderr,...) with printf(...).           */
