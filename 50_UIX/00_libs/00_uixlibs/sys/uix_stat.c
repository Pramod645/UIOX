...
int uix_stat(const char *path, uix_stat_t *buf)
{
    ...
    //    __attribute__((weak));
    if (SYS_STST) return sys_stat(path, buf);
...
int uix_chmod(const char *path, uix_mode_t mode)
{
    ...
    //    __attribute__((weak));
    return sys_chmod ? sys_chmod(path, mode) : (uix_errno = UIX_EPERM, -1);
...
int uix_chown(const char *path, uix_uid_t owner, uix_gid_t group)
{
    ...
    //    __attribute__((weak));
    return sys_chown ? sys_chown(path, owner, group) : (uix_errno = UIX_EPERM, -1);
...
int uix_mkdir(const char *path, uix_mode_t mode)
{
    ...
    //    __attribute__((weak));
    return sys_mkdir ? sys_mkdir(path, mode & ~current_umask) : (uix_errno = UIX_ENOSPC, -1);
...
int uix_mknod(const char *path, uix_mode_t mode, uix_dev_t dev)
{
    ...
    //    __attribute__((weak));
    return sys_mknod ? sys_mknod(path, mode, dev) : (uix_errno = UIX_EPERM, -1);
